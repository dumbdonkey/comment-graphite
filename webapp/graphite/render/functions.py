...
#pydevd.settrace('10.212.83.122',port=9999,stdoutToServer=True,stderrToServer=True)
...
def summarize(requestContext, seriesList, intervalString, func='sum', alignToFrom=False):
  ...
  """
  if intervalString.find("d")!=-1 or intervalString.find("day")!=-1:
     return smartSummarize(requestContext, seriesList, intervalString, 'sum', "days");
    
  if intervalString.find("m") != -1 or intervalString.find("minute") != -1:
     return smartSummarize(requestContext, seriesList, intervalString, 'sum', "minutes");

  if intervalString.find("h") != -1 or intervalString.find("hour") != -1:
     return smartSummarize(requestContext, seriesList, intervalString, 'sum', "hours");

  if intervalString.find("s") != -1 or intervalString.find("second") != -1:
    return smartSummarize(requestContext, seriesList, intervalString, 'sum', "seconds");
